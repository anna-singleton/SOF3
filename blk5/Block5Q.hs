...
checkMarks = all checkStudent 
  where
    ...
    checkStudent (Student ys) = all checkOneMark ys
    checkOneMark :: (Module, Maybe Int) -> Bool
    checkOneMark (_,x) | x == Nothing = True
                    | otherwise = (fromJust x) >= 0 && (fromJust x) <= 100
...
checkPrereqs s@(Student xs) = all (\(x,_) -> (all (`elem` getCompletedModules s) (prereq x))) xs
  where
    getCompletedModules :: Student -> [Module]
    getCompletedModules (Student ms) = map fst $ filter (\(_,x) -> x /= Nothing) ms

student1, student2 :: Student
student1 = Student [(THE1, Just 45), (SOF1, Just 80), (THE2, Nothing)]
student2 = Student [(THE1, Just 45), (SOF1, Nothing), (THE2, Nothing)]
...
prop_listOrdered xs = isSorted xs == listOrdered xs
...
rModule :: Gen Module
rModule = elements [THE1, SOF1, THE2, SOF2, SYS1, DAT1, HCI1]
